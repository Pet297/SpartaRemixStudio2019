    ...
    public partial class PatternPicker : UserControl
    {
        ...
        private void PatternPicker_Paint(object sender, PaintEventArgs e)
        {
            if (Form1.Project != null)
            {
                ...
                int posy = Yoffset;
                int index = 0;
                foreach (KeyValuePair<uint, Pattern> k in Form1.Project.ProjectPatterns)
                {
                    ...
                    /*if (k.Value.ivs.PreviewBitmap != null)
                    {
                        e.Graphics.DrawImage(k.Value.ivs.PreviewBitmap, new Rectangle(0, posy, ItemHeight * 4 / 3, ItemHeight));
                    }*/
                    e.Graphics.DrawString(k.Key.ToString(), new Font("Arial", 10), Brushes.Black, 0, posy);
                    if (k.Value.Name != null) e.Graphics.DrawString(k.Value.Name, new Font("Arial", 10), Brushes.Black, ItemHeight * 4 / 3 + 3, posy);
                    else e.Graphics.DrawString("(PATTERN " + k.Key.ToString() + ")", new Font("Arial", 10), Brushes.Black, ItemHeight * 4 / 3 + 3, posy);
        ...
        private void UpdateScrollBarHeight()
        {
            if (Form1.Project != null)
            {
                int i = (Form1.Project.ProjectPatterns.Count + 2) * ItemHeight - Height;
        ...
        private void PatternPicker_Click(object sender, EventArgs e)
        {
        ...
        public uint GetPattern()
        {
            ...
            else if (itemSelected >= Form1.Project.ProjectPatterns.Count) return uint.MaxValue;
            else
            {
                int left = itemSelected;
                foreach (uint u in Form1.Project.ProjectPatterns.Keys)
                {
                    if (left == 0) return u;
                    left--;
                }
